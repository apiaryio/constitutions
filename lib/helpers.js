...
function getAllRules(rulesDir = defaultRulesDir) {
  return _.map(glob.sync(rulesDir + '/*'), function(rule) {
    return _.last(rule.split('/'))
  });
}
...
function getRulesNames(allRules, allowedRules) {
  ...
  if (allowedRules) {
    _.each(allowedRules, function(rule) {
...
function testRuleStructure (ruleName, rulesDir = defaultRulesDir) {
  ...
  const getExamplesConf = (exampleDir, type) => {
    if (fs.existsSync(exampleDir)) {
      _.each(glob.sync(exampleDir +'/*/'), function(dir) {
  ...
  _.each(toCheck, function(check, key) {
    ...
    if (key !== 'examples' ) {
      ...
      _.each(_.concat(check.good, check.bad), function(example) {
...
function getRules(configFile = './build/build.yaml') {
  let config;
  let rules = {
    title: '',
    description: '',
  ...
  _.each(getRulesNames(getAllRules(), allowedRules), function(ruleName) {
