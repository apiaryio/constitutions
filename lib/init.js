...
function createNewRuleStructure(name, rulesDir = defaultRulesDir) {
  ...
  const exampleBaseDir = path.join(dir, 'examples');
  const badExampleBaseDir = path.join(exampleBaseDir, 'bad');
  const goodExampleBaseDir = path.join(exampleBaseDir, 'good');
